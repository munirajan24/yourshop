...
public class RecycleAdapter extends RecyclerView.Adapter<RecycleAdapter.CViewHolder> implements Filterable {
    ...
    public RecycleAdapter(List<RecycleItem> items, OnRecycleListener onRecycleListener) {
        this.items = items;
        this.onRecycleListener = onRecycleListener;
    }

    public RecycleAdapter(List<RecycleItem> items, OnRecycleListener onRecycleListener, boolean whiteLabel) {
        this.items = items;
        this.onRecycleListener = onRecycleListener;
        this.whiteLabel = whiteLabel;
    }

    public RecycleAdapter(List<RecycleItem> items, OnRecycleListener onRecycleListener, OnRecycleLongClickListener onRecycleLongClickListener) {
        this.items = items;
        this.onRecycleListener = onRecycleListener;
        this.onRecycleLongClickListener = onRecycleLongClickListener;
    }

    public RecycleAdapter(List<RecycleItem> items, OnRecycleListener onRecycleListener, OnRecycleLongClickListener onRecycleLongClickListener, Context mContext) {
        this.items = items;
        this.onRecycleListener = onRecycleListener;
        this.onRecycleLongClickListener = onRecycleLongClickListener;
    ...
    public CViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        RecycleItemBinding itemBinding = RecycleItemBinding.inflate(inflater, parent, false);
...
//            holder.binding.cateTitle.setTextColor(ContextCompat.getColor(holder.binding.cateTitle.getContext(), R.color.ms_white));
            ...
            if (items.get(position).getIs_enable().equals("0")) {
                holder.binding.cateTitle.setTextColor(ContextCompat.getColor(holder.binding.cateTitle.getContext(), R.color.transparent_40));
                holder.binding.cateTitle.setShadowLayer(2.5f, -1, 1, Color.WHITE);
            ...
            if (items.get(position).getStatus().equals("0")) {
                holder.binding.cateTitle.setTextColor(ContextCompat.getColor(holder.binding.cateTitle.getContext(), R.color.md_red_400));
            }else {
                holder.binding.cateTitle.setTextColor(ContextCompat.getColor(holder.binding.cateTitle.getContext(), R.color.md_grey_700));
            }
    ...
    public class CViewHolder extends RecyclerView.ViewHolder {
        ...
        public CViewHolder(@NotNull RecycleItemBinding binding) {
